...
void
init_ans(std::vector<std::string>& ans) {
	for (int i = 1; i <= MAX_N; i++) {
		std::vector<bool> binaries = get_binaries(i);
		std::string ans[i] = to_binary_string(binaries);
	}
}

int
main() {
	std::vector<std::string> ans(MAX_N + 1);
	init_ans(ans);
