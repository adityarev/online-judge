...
void
set_counter(std::unordered_map<std::string,int>& counter, int& n) {
	for (int i = 0; i < n; i++) {
...
int
get_ans(std::unordered_map<std::string,int>& counter) {
	int ans = 0;
...
run_case(int c) {
	...
	set_counter(counter, n);
	
	int ans = get_ans(counter);
	printf("Kasus #%d: %d\n", c, ans);
}
