...
get_ans(
	...
	travel.push({{start, 0}, vector<int>(v, INT_MAX)});
	while (!travel.empty()) {
		CheckPoint cp = travel.top();
		travel.pop();
		
		min_dist[cp.node.id] = cp.node.dist;
		(cp.min_dist)[cp.node.id] = cp.node.dist;
		
		if (cp.node.id == dest)
			return (cp.min_dist)[cp.node.id];
		
		for (Node& adj: adjs[cp.node.id]) {
			int dist = min_dist[cp.node.id] + adj.dist;
			
			if ((cp.min_dist)[adj.id] != INT_MAX && dist <= (cp.min_dist)[adj.id])
				return INT_MIN;
			if (dist < min_dist[adj.id])
				travel.push({{adj.id, dist}, cp.min_dist});
		}
	}
